...
// ============================================================================
// Token 撤销名单（KEY: revoked:{jti}，存在即撤销，带 TTL）
// ============================================================================
//
...
export async function revokeToken(kv, jti, ttlSeconds) {
  const key = `revoked_${jti}`;
  // TTL 至少 60 秒，避免传入 0/负数导致写入异常
 ...
 * @param {KVNamespace} kv - EdgeOne KV 实例
 * @param {string} jti - Token 唯一标识
 * @returns {Promise<boolean>} true=已撤销，false=有效（默认）
 */
